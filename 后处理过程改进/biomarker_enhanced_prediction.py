...
class BiomarkerEnhancedPredictor:
    ...
    def __init__(self, biomarker_dir: str, 
        ...
        min_biomarker_confidence : float
            生物标志物最小置信度阈值
        """
        self.biomarker_matcher = BiomarkerMatcher(biomarker_dir)
...
def create_biomarker_enhanced_predictor(biomarker_dir: str,
                                       model_predictor,
                                       biomarker_weight: float,
                                       min_biomarker_confidence: float = 0.6) -> BiomarkerEnhancedPredictor:
    """
    创建生物标志物增强预测器的便捷函数
...
def evaluate_accuracy(data_windows, net, device_acc,bool_use):
    pre_counts = 0
    with torch.no_grad():
        slide_i = 0
...
if __name__ == "__main__":
    ...
    # 这里需要根据实际情况提供模型预测函数和生物标志物目录
    # 示例代码：

    def dummy_model_predictor(sample):
        # 返回 [P(Interictal), P(Preictal)]
        return np.array([0.7, 0.3])

    biomarker_dir = r"D:\2025_important_projects\data\chbmit_biomarkers\01\preprocess_eeg"
    ...
    for sub_dir_i in tqdm(range(len(data_sub_dirs))):
        sub_dir = data_sub_dirs[sub_dir_i]
        sub_patient_dirs = os.path.join(data_root_dir, sub_dir)
        data = np.load(sub_patient_dirs)
        ...
        while (j + 1) * 15360 <= data.shape[-1]:
            ...
            data_counts = evaluate_accuracy(split_data, net=model, device_acc=device,bool_use=True)
        ...
        # 将键和值添加到字典中
        each_patient_subarrays[sub_dir] = subarrays
        break
